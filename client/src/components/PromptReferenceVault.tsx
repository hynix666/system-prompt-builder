...
import { FileText, FolderOpen, Loader2, LogIn, Paperclip, Trash2, Upload } from "lucide-react";
...
export function PromptReferenceVault() {
    ...
    <section className="sl-reference-vault" aria-labelledby="reference-vault-title">
      ...
      {!loading && !isAuthenticated ? (
        ...
        <>
          ...
          {assets.isLoading ? <p className="sl-reference-status"><Loader2 className="sl-reference-spin" size={13} /> Loading stored files…</p> : assets.data?.length ? <ul className="sl-reference-list">{assets.data.map((asset) => <li key={asset.id}><a href={asset.url} target="_blank" rel="noreferrer"><FileText size={13} /><span>{asset.originalName}</span><small>{readableSize(asset.byteSize)}</small></a><button type="button" aria-label={`Remove ${asset.originalName}`} disabled={remove.isPending} onClick={() => remove.mutate({ id: asset.id })}><Trash2 size={13} /></button></li>)}</ul> : <p className="sl-reference-empty"><Paperclip size={14} /> No files stored yet.</p>}
        </>
      )}
    </section>
  );
